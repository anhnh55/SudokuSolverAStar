...
struct compare_f_value
{
	bool operator()(const SudokuBoard b1, const SudokuBoard b2) const
	{
		...
		if(b2.m_i_value_f == b1.m_i_value_f)
		{
			...
			if(b2.m_i_total_candidates < b1.m_i_total_candidates)
				return true;
			if(b2.m_i_total_candidates == b1.m_i_total_candidates)
			{
				return b2.m_i_remaining_blank < b1.m_i_remaining_blank;
			}
		}
		return false;
	}
...
priority_queue<SudokuBoard,vector<SudokuBoard>,compare_f_value> open_set; //set of node waiting to be expanded
...
int main ()
{
	...
	//generateSuccessorAt(start_board,0,0);
